...
interface SeriesRowProps {
  ...
  onPlay: (serie: Midia) => void
  limit?: number
}

export function SeriesRow({ categoria, credentials, onPlay, limit = 20 }: SeriesRowProps) {
  const [series, setSeries] = useState<Midia[]>([])
    ...
    const fetchSeries = async () => {
      try {
        ...
        if (Array.isArray(data)) {
          setSeries(data.slice(0, limit))
        }
      } catch (e) {
        console.error('Erro na fileira de séries', e)
      } finally {
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: '8px', marginBottom: '32px' }}>
      ...
      >
        {series.map((serie) => (
          <div key={serie.id} style={{ flexShrink: 0, width: '140px' }}>
            <SeriesCard movie={serie} onClick={onPlay} />
          </div>
        ))}
      </div>
    </div>
